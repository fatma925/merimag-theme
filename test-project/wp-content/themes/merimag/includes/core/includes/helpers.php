...
/**
 * CSS 3 CSS VARS GENERATOR
 *
 * @param array $varsx
 * @param string $area valid css selector
 * @return array $defaults list of defaults values
 */
function merimag_get_area_css_vars( $vars = array(), $area, $defaults = array() ) {
  $css = '';
  if( is_array($defaults) && count( $defaults ) >= count( $vars ) ) {
    foreach( $defaults as $default_name => $default_val ) {
      $new_name = str_replace('_', '-', $default_name );
      $defaults[$new_name] = $default_val;
      unset($defaults[$default_name]);
    }
    $vars = array_replace($defaults, $vars );
  }
  if( is_array( $vars ) && !empty( $vars ) ) {
    ...
    foreach( $vars as $var_name => $value ) {
      if( $value ) {
        ...
        $css .= '--' . esc_attr( $var_name ) . ':' . esc_attr($value) . ';';
        $css .= $var_name == 'text-color' ? 'color: var(--text-color);' : '';
      }
    }
    $css .= '}';
  }
...
function merimag_render_css( $css, $element_id = '' ) {
  if( !is_string($css) ) {
    return;
  }
  $css_tag  = 'sty' . 'le';
  $css_id = $element_id ? $element_id . '-inline-css' : '';
  echo sprintf('<' . esc_attr( $css_tag ) . ' type="text/css" id="%s">', esc_attr($css_id ) );
    ...
    if( is_array( $args ) && $posts  ) {
      foreach( $posts as $post ) {
        ...
        $elements[] = (object) array(
          ...
          'description' => strip_shortcodes(wp_trim_words(get_the_content( null, false, $post->ID), 120, '...')),
      ...
      if( !is_array( $args ) && have_posts() ) {
        while ( have_posts() ) : the_post();
          $elements[] = (object) array(
            ...
            'description' => strip_shortcodes(wp_trim_words(get_the_content(), 120, '...')),
...
function merimag_meta_info( $parse = '', $id = false, $multi_line = false ) {
  ...
  $check_meta = isset( $meta_infos[$parse] ) ? $meta_infos[$parse] : false;
    ...
    $show_author = merimag_get_db_customizer_option('show_author', 'yes');
      ...
      switch ($element) {
        ...
        case 'views':
          ...
          if( $views && is_numeric( $views ) ) {
            $html .=  sprintf('<span class="merimag-block-infos-views %s"><i class="%s"></i> %s</span>', esc_attr( $class ), esc_attr($icon), esc_attr( $views ) );
          }
          break;
        case 'views_ajax':
          $views = merimag_get_post_views( $id );
          ...
          $class = sprintf('%s-color-color', $badge);
          if( $views && is_numeric( $views ) ) {
            $html .=  sprintf('<span class="merimag-block-infos-views %s"><i class="%s"></i> <span class="merimag-post-views" data-id="%s"><i class="animate-spin icon-spinner2"></i></span></span>', esc_attr( $class ), esc_attr($icon), esc_attr( $id ) );
          }
          break;
        case 'categories':
...
function merimag_ajax_post_views() {
  $id = $_POST['id'];
  $views = merimag_get_post_views( $id );
  if( $views && is_numeric( $views ) ) {
    echo $views;
...
add_action( 'wp_ajax_merimag_ajax_post_views', 'merimag_ajax_post_views' );
add_action( 'wp_ajax_nopriv_merimag_ajax_post_views', 'merimag_ajax_post_views' );
/**
 * Display multiple meta infos parsed from a string separed with |
 *
 * @param string $parse string that contains list of meta infos separed with |
...
function merimag_get_post_views( $id = false ) {
  ...
  if( $fake_post_views === 'yes' ) {
    ...
    $views = get_post_meta( $id, 'entry_views', true );
    return $views;
  }
}
/**
...
function merimag_get_thumbnail_background_tag( $atts = array() ) {
  ...
  $class               .= isset( $atts['copyrights'] ) && $atts['copyrights'] ? ' merimag-image-has-copyrights' : '';
  ...
  if( isset( $atts['copyrights'] ) && $atts['copyrights'] ) {
    $html .= sprintf('<span class="merimag-image-copyrights">&copy; %s</span>', esc_attr( $atts['copyrights'] ) );
  }
  return $html;
}
...
function merimag_get_recognized_social_networks( $keys = false, $get = 'name' ) {
  ...
  $social_networks = array(
      ...
      'telegram'=> array( 'color' => '#0088cc', 'name' => __('Telegram', 'merimag') , 'icon' => 'icofont-telegram', 'title' => __('Telegram', 'merimag'), 'action' => __('Follow us', 'merimag') ),
...
function merimag_get_posts( $post_type = 'post', $default_choice = false ) {
  $cached_posts = get_transient('merimag_cache_get_posts_' . $post_type );
  if( $cached_posts && !empty($cached_posts) && !empty( $cached_posts ) ) {
     $choices = $cached_posts;
  } else {
    ...
    set_transient('merimag_cache_get_posts_' . $post_type, $choices );
  }
  return $choices;
}
// Cache get terms
...
function merimag_get_demo_ad( $size = '300x250' ) {
  ...
  return '<div style="width: '. esc_attr($width) . 'px" class="merimag-demo-ad ' . esc_attr( $class ) . '"><a target="_blank" href="' . esc_url( $buy_url ) . '"><img src="' .esc_url( $url ) . '"></a><a  target="_blank" href="' . esc_url( $buy_url ) . '" class="merimag-demo-ad-button principal-color-background-color">' . strtoupper($name) .'</a></div>';
}
// Check ajax referer
