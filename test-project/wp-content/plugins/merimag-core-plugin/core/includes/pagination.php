...
class merimagPaginationLinks {
  ...
  public static function create(
      ...
      $numberOfPages,
      $context    = 1,
      $linkFormat = '<a class="merimag-button" href="?page=%d">%d</a>',
      $pageFormat = '<span class="merimag-button merimag-button-disabled">%d</span>',
      $ellipsis   = '<span class="merimag-button">&hellip;</span>',
      $next_prev  = true){
    ...
    $prev_class = $page === 1 ? 'merimag-button-disabled' : '';
    $next_class = $page == $numberOfPages ? 'merimag-button-disabled' : '';
    $nextFormat = '<a class="next merimag-button %s" href="#">%s</a>';
    $prevFormat = '<a class="prev merimag-button %s" href="#">%s</a>';

    if( $next_prev === true ) {
      array_unshift( $links, sprintf( $prevFormat, $prev_class, esc_html__('Previous', 'merimag') ) );
      array_push( $links, sprintf( $nextFormat, $next_class, esc_html__('Next', 'merimag') ) );
    }
    // return the links
