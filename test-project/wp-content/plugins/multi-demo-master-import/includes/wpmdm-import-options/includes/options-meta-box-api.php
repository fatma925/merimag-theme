 ...
 */
if ( ! class_exists( 'wpmdm_import_options_meta_box' ) ) {

  class wpmdm_import_options_meta_box {
     ...
     *
     * This method adds wpmdm_import_optionsher methods of the class to specific hooks within WordPress.
     *
     * @uses      add_action()
     *
     * @return    void
    ...
    function __construct( $meta_box ) {
      ...
      if ( ! isset( $wpmdm_import_options_meta_boxes ) ) {
        $wpmdm_import_options_meta_boxes = array();
      }

      $wpmdm_import_options_meta_boxes[] = $meta_box;
    ...
    function build_meta_box( $post, $metabox ) {
      ...
      echo '<div class="wpmdm-import-options-metabox-wrapper">';
       ...
       wpmdm_import_options_js_wp_editor();
       ...
       if( empty( $this->meta_box['fields'] ) ) {
        ...
        foreach ( (array) $this->meta_box['fields'] as $field ) {
          ...
          $_args = array(
            ...
            'post_id'           => $post->ID,
            'meta'              => true,
            'message'           => isset( $field['message'] ) ? $field['message'] : array(),
          );
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
        ...
        if ( isset( $_POST[$field['id']] ) ) {
          ...
          if ( in_array( $field['type'], array( 'list-item', 'slider' ) ) ) {
            ...
            $settings = isset( $_POST[$field['id'] . '_settings_array'] ) ? unserialize( wpmdm_import_options_decode( $_POST[$field['id'] . '_settings_array'] ) ) : array();
            
            /* settings are empty for some odd ass reason get the defaults */
            if ( empty( $settings ) ) {
              $settings = wpmdm_import_options_list_item_settings( $field['id'] );
            }
            ...
            foreach( $_POST[$field['id']] as $k => $setting_array ) {
              ...
              foreach( $settings as $sub_setting ) {
                ...
                if ( isset( $sub_setting['type'] ) && isset( $_POST[$field['id']][$k][$sub_setting['id']] ) ) {
                  ...
                  $_POST[$field['id']][$k][$sub_setting['id']] = wpmdm_import_options_validate_setting( $_POST[$field['id']][$k][$sub_setting['id']], $sub_setting['type'], $sub_setting['id'] );
                  
                }
                
                if($sub_setting['type']=='list-item' && isset($_POST[$field['id'].'_'.$sub_setting['id'].'_'.$k])) {
                  
                  
                  $inner_settings = isset( $_POST[$field['id'].'_'.$sub_setting['id'].'_'.$k . '_settings_array'] ) ? unserialize( wpmdm_import_options_decode( $_POST[$field['id'].'_'.$sub_setting['id'].'_'.$k . '_settings_array'] ) ) : array();
                  foreach( $inner_settings as $si => $sub_inner_setting ) {
                    /* get the settings array */

                      $_POST[$field['id']][$k][$sub_setting['id']]= $_POST[$field['id'].'_'.$sub_setting['id'].'_'.$k];
            ...
            if ( empty( $settings ) ) {
              $settings = wpmdm_import_options_social_links_settings( $field['id'] );
            }
            ...
            $new = wpmdm_import_options_validate_setting( $_POST[$field['id']], $field['type'], $field['id'] );
          ...
          if ( $field['type'] == 'css' ) {
            ...
            if ( '' !== $new ) {
              ...
              wpmdm_import_options_insert_css_with_markers( $field['id'], $new, true );
            
            /* remove old CSS from dynamic.css */
            } else {
            
              wpmdm_import_options_remove_old_css( $field['id'] );
 ...
 *
 * @uses     wpmdm_import_options_meta_box()
 *
...
if ( ! function_exists( 'wpmdm_import_options_register_meta_box' ) ) {
  ...
  function wpmdm_import_options_register_meta_box( $args ) {
    if ( ! $args )
      return;
      
    $wpmdm_import_options_meta_box = new wpmdm_import_options_meta_box( $args );
  }

}

/* End of file wpmdm-import-options-meta-box-api.php */
/* Location: ./includes/wpmdm-import-options-meta-box-api.php */